...
public class MORDNOGenerator implements IdentifierGenerator{
	...
	public Serializable generate(SharedSessionContractImplementor session, Object object) throws HibernateException {
		...
		String prefix= sdate + "-";  //您要產生的自增主鍵字串
		Connection con= session.connection(); //產生連線
		try {
			PreparedStatement pstmt = con.prepareStatement("select MORD_seq.nextval from dual"); //從資料庫取的下一個SEQUENCE的值
			ResultSet rs = pstmt.executeQuery();
			if(rs.next()){
				int no = rs.getInt("nextval"); //將此次的值放到no的變數中
				String code = prefix + StringUtils.leftPad("" + no, 8, '0');  //將變數code裡放入要產生的自訂自增主鍵(此範例為OFF21XXXXXX)
